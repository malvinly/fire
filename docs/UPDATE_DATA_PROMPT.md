...
- **Pairing rule (read the comment in rules.ts):** the bend points for first-eligibility year E are derived
  ...
  points you use. SSA publishes AWI(Y−1) and the bend points for Y+1 together in October of year Y; if you
  are updating in October–December, you may use the newer pair — but update both or neither.
...
- Add a dated entry to the "Data update history" section at the end of `docs/DECISIONS.md` listing what
  changed. Old session files will now show "computed with older data — recalculate"; that is expected.
...
- Tell the user: open last year's session, **Save as new**, click the banner's "Start plan in <year>", update
  ...
  assumption; if the Trustees numbers changed, use Assumptions → "Reset assumptions to defaults" (or edit the
  four trust-fund fields) in each session you carry forward.
