...
| D17 | **Dated items apply only from the retirement date on.** Anything dated earlier — including inflows such as a home sale — is ignored; the UI says so. | Avoids double-counting expenses (e.g. a mortgage) already paid from salary. **Leans optimistic** for future costs that are not in today's budget (a roof, a car, college before retirement): they are dropped, and for Coast FIRE that covers every year until the coast age. Subtract such costs from today's savings by hand. A pre-retirement inflow (home sale) is also dropped, so retiring one year later can look worse; add it to the brokerage balance instead (this gives it growth it would not yet have had, a small optimistic effect). |
...
| D47 | **Two kinds of historical check.** Whole-plan results (earliest date, success today, the detail view, worst-years table) replay the plan from the plan start: "start year 1966" means the plan's first year behaves like 1966. The FIRE *number* is a retirement-only test, so its history leg uses windows that start at the retirement year. Worst-years table: failures first, earliest failure first, then lowest ending balance. | Each check matches the question it answers (D54). Caveat: in the whole-plan replay each bad retirement start (1929, 1966) is reached after the years that preceded it in history (often a boom), so the date's history check is gentler on those years than the FIRE number's. The simulated-markets check does not share this pairing, and the lower of the two counts. | `src/engine/solve.ts` `histFor` |
