...
| D62 | Results opened from a session file are labelled as saved results with a **Recalculate** button, and damaged session files are rejected when opened. | A session file can be edited or come from someone else; its results must not pass for the app's own calculation. | `src/App.tsx`, `src/ui/sessions.ts` |
...
|---|---|---|
| *(core)* 90% target, age 96 of the younger spouse, stricter-of-two, no ACA subsidies, healthcare +1.5%/yr, trust-fund cut; D5 re-check only moves answers later; D7 withdraw first; D14 59½ as age 60; D15 over-limit savings dropped; D24 no real wage growth in SS; D32 no senior bonus deduction; D39 fill can cost 15% on gains; D40 no losses; D41 HSA penalty to 65 of the younger; D17 a pre-retirement cost savings can't cover fails the path | D34 no yearly dividend tax; D25 no earnings test; D13 both spouses alive (spending never drops); D7 contributions also land at the start of the year and earn a full year's return; D15 real wage growth continues until work stops at any age; D32 indexed tax amounts held constant in CPI terms although the IRS indexes them to the slower chained CPI (~0.25%/yr), so brackets and the fill room are slightly too wide on long plans | D42/D51 average-return projection for the FIRE number's account mix; US-only data (history's big winner — optimistic in a global sense) |
