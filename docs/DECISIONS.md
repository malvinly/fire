...
| D48 | Example numbers on first launch are placeholders (two people aged 42/40, $100k salaries). Chubby FIRE spending defaults to **1.0 × (current spending − ongoing dated expenses already being paid today)** — the D18 base without the 15% cut; clearing it skips the Chubby result. | Online definitions of Chubby FIRE are dollar bands (~$80k–$200k/yr), not a ratio; described as a lifestyle it means keeping today's comfort without belt-tightening (Lean below, Fat above). 1.0× is the plainest reading of that; anything higher drifts toward Fat FIRE. Same base as D18 so a mortgage isn't counted twice. | `src/engine/defaults.ts` `CHUBBY_SPENDING_FACTOR` |
