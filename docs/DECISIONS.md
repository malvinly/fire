...
| D48 | Example numbers on first launch are placeholders (two people aged 42/40, $100k salaries). Chubby FIRE spending defaults to **1.2 × (current spending − ongoing dated expenses already being paid today)** — the D18 base; clearing it skips the Chubby result. | Online definitions of Chubby FIRE are dollar bands (~$80k–$200k/yr), not a ratio; described as a lifestyle it is comfortable, not frugal, but short of Fat FIRE's no-budget luxury (roughly 1.5×+ today's spending). 1.0× landed close to Traditional (0.85×); 1.2× is a clear step up from today while staying well below Fat. Same base as D18 so a mortgage isn't counted twice. | `src/engine/defaults.ts` `CHUBBY_SPENDING_FACTOR` |
