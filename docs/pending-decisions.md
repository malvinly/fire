...
- **Blocks:** fix 17 (whether to scale SS up or only state the size in D24). It also frames features 1, 5,
  7 and 8.
...
(separate state rate) all add inputs. Fix 2 shows the current input count already lets example values leak
...
- **Options:**
  ...
  - (c) limit v2 to the highest-impact inputs (features 1–2).
- **Blocks:** nothing outright. It shapes the UI for fix 3 and features 2, 4 and 7.
...
But v1 doesn't model what conversions protect against: survivor single-filer brackets (feature 6), IRMAA,
...
- **Options:**
  ...
  - (c) keep 12% and show the comparison (feature 3);
  - (d) decide after feature 6, when the survivor case can show conversions' benefit.
- **Blocks:** feature 3's presentation; any change to D28/D29. Fix 5's help-text correction doesn't need to
  wait.
...
- Adding Rule of 55 (feature 8) would move them earlier, by removing penalties that wouldn't really apply.
- **Options:**
  - (a) keep as is;
  - (b) add a threshold (e.g. penalties above X% of a year's spending count as failure);
  - (c) decide after feature 8.
- **Blocks:** nothing directly. It interacts with fix 6 and feature 8.
...
- **Blocks:** the exact wording of fixes 4, 12 and 18. Feature 5's report format.
